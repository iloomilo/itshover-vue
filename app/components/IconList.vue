  ...
  >
    ...
    <AnimatePresence mode="wait">
      <motion.div
        ...
        animate="visible"
      >
        <motion.div v-for="icon in filteredIcons" :key="icon.name" :variants="itemVariants" layout>
          <IconCard :icon="icon" />
        </motion.div>
        ...
        <motion.div
          ...
          :transition="{ duration: 0.4, delay: 0.1 }"
        >
          <p class="text-muted-foreground text-sm">No icons found for "{{ searchQuery }}"</p>
        </motion.div>
      </motion.div>
    </AnimatePresence>
  </motion.div>
