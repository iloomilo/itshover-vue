...
const start = () => {
  animate('.dolphin', { y: [0, -2, 0] }, { duration: 0.8, ease: 'easeInOut' })
  animate('.eye', { scale: [1, 1.5, 1] }, { duration: 0.3, ease: 'easeInOut' })
}

const stop = () => {
  animate('.dolphin, .eye', { y: 0, scale: 1 }, { duration: 0.2, ease: 'easeInOut' })
}
...
<template>
  ...
  >
    ...
    />
    <path class="eye" :style="{ transformOrigin: '9 7' }" d="M9 7h.01" />
  </svg>
</template>
