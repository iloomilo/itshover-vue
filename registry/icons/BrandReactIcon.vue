...
const start = async () => {
  ...
  animate('.orbit-2', { scale: [1, 1.03, 1] }, { duration: 3.5, ease: 'easeInOut', delay: 0.5 })
  animate('.orbit-3', { scale: [1, 1.02, 1] }, { duration: 4, ease: 'easeInOut', delay: 1 })
}

const stop = async () => {
  // Smooth deceleration
  await animate('.orbit-1', { rotate: 0, scale: 1 }, { duration: 0.8, ease: [0.22, 1, 0.36, 1] })
  await animate('.orbit-2', { rotate: 0, scale: 1 }, { duration: 0.8, ease: [0.22, 1, 0.36, 1] })
  await animate('.orbit-3', { rotate: 0, scale: 1 }, { duration: 0.8, ease: [0.22, 1, 0.36, 1] })

  // Center dot settles back
  await animate('.center-dot', { scale: 1, opacity: 1 }, { duration: 0.5, ease: 'easeOut' })
}
...
<template>
  ...
  >
    ...
    <g class="orbit-2" :style="{ transformOrigin: '50% 50%' }">
      <path
    ...
    <g class="orbit-3" :style="{ transformOrigin: '50% 50%' }">
      <path
