...
const start = () => {
  animate('.eye-left', { scaleY: [1, 0.1, 1] }, { duration: 0.3, ease: 'easeInOut' })
  animate('.eye-right', { scaleY: [1, 0.1, 1] }, { duration: 0.3, ease: 'easeInOut', delay: 0.1 })
}

const stop = () => {
  animate('.eye-left, .eye-right', { scaleY: 1 }, { duration: 0.2, ease: 'easeOut' })
}
...
<template>
  ...
  >
    ...
    >
      ...
      />
      <path class="eye-left" d="M10 15v2" :style="{ transformOrigin: '10px 16px' }" />
      <path class="eye-right" d="M14 15v2" :style="{ transformOrigin: '14px 16px' }" />
    </svg>
  </div>
</template>
